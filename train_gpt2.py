...
total_batch_size = 524288 # 2**19, 0.5M, in nums of tokens
B = 16
T = 1024
assert total_batch_size % (B * T * ddp_world_size) == 0 # make sure the batch size is divisible by B * T * ddp_word_size
...
for step in range(max_steps):
    ...
    t1 = time.time()
    dt = (t1 - t0) # time differenes in s
    tokens_per_sec = (train_loader.B * train_loader.T * grad_accum_steps * ddp_world_size) / dt
    if master_process:
        print(f"step {step:4d} | lr: {lr:.3e} | loss: {loss_accum.item():.6f} | norm: {norm:.4f} | dt: {dt*1000:.2f} ms | tok/sec: {tokens_per_sec:.2f}")
...
# prefix token
tokens = enc.encode("Hello, I am a doctorate student at Harvard")
